"""Generate the dissertation figures as vector PDFs from data/ outputs."""
...
FIGS = str(Path(__file__).resolve().parents[1] / "figures")
...
def fig_tpc():
    ...
    for ax, (nt, title) in zip(axes, notations):
        ...
        ax.set_yticks(y, [LABEL[t] for t in ORDER], fontsize=8)
        ax.set_title(title, fontsize=9, color=INK, pad=4)
        ax.set_xlim(0, 1.42)
        ax.set_xticks([0, 0.5, 1.0])
        style_ax(ax, xgrid=True)
    axes[1].set_xlabel("tokens per character (lower is more compact)", fontsize=8)
...
def fig_fen_fields():
    ...
    for ax, M, U, cols, labels, title in (
        ...
        ax.set_title(title, fontsize=7.5, color=MUTED, style="italic", pad=5)
        ax.tick_params(length=0)
        for s in ax.spines.values():
            s.set_visible(False)
        heat_grid(ax, M)

    axl.set_yticks(range(6), [LABEL[t] for t in ORDER], fontsize=8)
    axr.set_yticks([])
    fig.text(0.5, -0.10, "tokens per field (each panel on its own colour scale; "
             "darker is more tokens)", ha="center", fontsize=7.5, color=INK2)
    fig.tight_layout()
...
def fig_emission():
    df = pd.read_csv(rf"{DATA}/emission_by_category.csv")
    ...
    ax.set_xticks(range(len(ORDER)), [LABEL[t] for t in ORDER], fontsize=8)
    ax.set_yticks(range(len(cats)), clabel, fontsize=7.5)
    ax.tick_params(length=0)
    for s in ax.spines.values():
        s.set_visible(False)
    heat_grid(ax, M)
    ax.set_xlabel("per cent of units emitted as exactly one token "
                  "(a dot marks none)", fontsize=7.5, labelpad=7)
    fig.tight_layout()
    fig.savefig(rf"{FIGS}/fig_emission.pdf", bbox_inches="tight")
    plt.close(fig)
...
def fig_convergence():
    ...
    DASH = dict(zip(ORDER, [(0, (7, 2)), (0, (4, 1.5)), (0, (1, 1.2)), "-",
                            (0, (5, 1, 1, 1)), (0, (3, 1))]))
    handles = []
    for tk in ORDER:
    ...
    axl.set_ylabel("SAN tokens per character", fontsize=8)
    axl.set_xlabel("games in prefix (log scale)", fontsize=8)
    axl.set_title("absolute level", fontsize=7.5, color=MUTED,
                  style="italic", pad=5)
    style_ax(axl, ygrid=True)

    axr.axhspan(-1, 1, color="#eaf2fd", zorder=0)
    axr.axhline(0, color=AXIS, linewidth=0.8, zorder=1)
    axr.text(8000, 1.02, "one per cent adequacy criterion", fontsize=6.5,
    ...
    axr.set_ylabel("deviation from full-corpus value (%)", fontsize=8)
    axr.set_xlabel("games in prefix (log scale)", fontsize=8)
    axr.set_title("deviation against the criterion", fontsize=7.5, color=MUTED,
                  style="italic", pad=5)
    style_ax(axr, ygrid=True)

    fig.tight_layout()
    leg = fig.legend(handles=handles, fontsize=7.5, frameon=False, ncols=6,
...
def fig_seeds():
    ...
    ax.text(len(ORDER) - 0.55, 1.02, "one per cent criterion", fontsize=6.5,
            ha="right", va="bottom", color=INK2)
    ax.set_xticks(x, [LABEL[t] for t in ORDER], fontsize=8)
    ax.set_ylim(-1.35, 1.35)
    ax.set_xlim(-0.6, len(ORDER) - 0.4)
    ax.set_ylabel("SAN fertility, deviation\nfrom the seed-42 draw (%)", fontsize=8)
